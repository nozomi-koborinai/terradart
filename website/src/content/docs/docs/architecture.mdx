...
- **[`terradart_appwrite`](https://pub.dev/packages/terradart_appwrite)** — official `appwrite/appwrite` provider resources (filled at the current pin).
- **[`terradart_cloudflare`](https://pub.dev/packages/terradart_cloudflare)** — official `cloudflare/cloudflare` provider resources (filled at `5.23.0`: 257 resource factories + 446 data sources; nested plugin-framework objects are typed Dart helpers).
- **[`terradart_aws`](https://github.com/nozomi-koborinai/terradart/tree/main/packages/terradart_aws)** — HashiCorp [`aws`](https://registry.terraform.io/providers/hashicorp/aws/latest) provider resources (filled at `6.66.0`: 1725 resource factories + 683 data sources; not on pub.dev yet). See [Dart apps on AWS](/docs/aws/).
...
- [Dart apps on AWS](/docs/aws/) — `terradart_aws` for Lambda, ECS Express Mode, and S3 + CloudFront.
