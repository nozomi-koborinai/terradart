...
**Output** — an object with a `barrels` array of `{ name, resource_count }`, one entry per barrel (118 in total, including the `data` barrel):
...
Lists curated resources and data sources. Pass `barrel` to filter to one service, or omit it to list all 1154.
