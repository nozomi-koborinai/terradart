...
- `terradart_leftover.tf` — resources, data sources, module calls, `moved` blocks, provider aliases;
...
Nothing the Stack owns is repeated (its `required_providers`, its provider configurations, its backend, the variables it declares), because Terraform rejects those twice. What keeps a block in Terraform today: a type outside the four curated catalogs, `count` / `for_each` / `dynamic` / `provisioner` / `timeouts`, a provider alias, an argument with no Dart parameter, an expression on a non-string argument, a sensitive literal (never copied into Dart), and `depends_on` on a resource that stays in Terraform. `MIGRATION.md` lists every kept block with its reason and file.
...
The blockers above shrink as the runtime grows — expressions on non-string arguments ([#662](https://github.com/nozomi-koborinai/terradart/issues/662)), provider aliases ([#666](https://github.com/nozomi-koborinai/terradart/issues/666)), `moved` and `count` / `for_each` ([#663](https://github.com/nozomi-koborinai/terradart/issues/663)), module calls ([#665](https://github.com/nozomi-koborinai/terradart/issues/665)) — followed by `--merge-envs` ([#668](https://github.com/nozomi-koborinai/terradart/issues/668)) and incremental re-runs ([#669](https://github.com/nozomi-koborinai/terradart/issues/669)). Progress is tracked on the [migrator epic](https://github.com/nozomi-koborinai/terradart/issues/80).
