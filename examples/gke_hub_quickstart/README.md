...
End-to-end terradart example for GKE Hub fleet team management — **no cluster required**. Enables the GKE Hub API and provisions a fleet scope (`terradart-scope`) and a fleet namespace (`terradart-team`) inside it — and exports the scope id as a typed Dart constant.
...
├── lib/main.dart       # FleetStack (API enablement + fleet scope + namespace)
...
Fleet scopes and namespaces are free fleet-management resources; the stack creates and destroys cleanly in a single project.
