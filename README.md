...
[`terradart_google`](packages/terradart_google/README.md) ships **620 curated resource factories + 1 data source** (621 catalog entries): AlloyDB (4 — cluster + instance + user + backup), API Keys (1 — key), App Engine (8 — application + URL dispatch + domain mapping + firewall rule + standard/flex versions + service network/traffic settings), Artifact Registry (4 — project config + repository + repository IAM member + download rule), Bigtable (10 — instance + table + app profile + GC policy + authorized/logical/materialized views + schema bundle + instance/table IAM members), Binary Authorization (3 — policy + attestor + attestor IAM member), Blockchain Node Engine (1 — node), Container Analysis (3 — note + note IAM member + occurrence), BigQuery (26 — standalone dataset access + dataset / table / dataset_iam_member / table_iam_member + job / routine / routine_iam_member / binding / policy / data_transfer_config / reservation / reservation group / capacity_commitment / connection + assignment / row access / data policy / datapolicy IAM + Analytics Hub exchange/listing/IAM/subscription + BI reservation + connection IAM), Chronicle (4 — custom list + native dashboard + dashboard chart + environment), Certificate Manager (6 — DNS authorization / certificate / certificate map / map entry / trust config / issuance config), Private CA (5 — CA pool + pool IAM member + certificate authority + certificate + certificate template), Public CA (1 — ACME external account key), Cloud Build (4 — v2 SCM connection / v2 repository / trigger + worker pool), Config (1 — deployment), Cloud Deploy (6 — delivery pipeline / target / custom target type + their IAM members), Cloud Functions (2), Cloud IDS (1 — endpoint), Cloud Run v2 (6), Cloud Scheduler (1), Cloud SQL (5 — instance / database / user + SSL cert + source representation), Cloud Tasks (2), Contact Center Insights (8 — encryption spec + analysis rule + view + QA scorecard / revision / question + assessment rule + auto-labeling rule), Container / GKE (7 — cluster + node pool + fleet + membership + fleet scope + fleet namespace + hub feature), GKE on AWS (2 — cluster + node pool), GKE on Azure (3 — client + cluster + node pool), GKE attached clusters (1 — cluster), GKE Backup (6 — plans + channels + plan IAM members), API Hub (1 — instance), Apigee (30 — organization + instance + instance attachment + environment + envgroup + envgroup attachment + environment addons + org addons + data collector + datastore + NAT address + endpoint attachment + DNS zone + space + sync authorization + target server + developer + developer app + API product + app group + env keystore + env references + environment KVM + KVM entries + keystore self-signed cert alias + API deployment + control plane access + environment API revision deployment + security feedback + security profile v2), Backup and DR (6 — vault + plan + association + management server + service config + restore), BeyondCorp (5 — app gateway + connector + connection + security gateway + security gateway application), App Hub (5 — application + boundary + service + service-project attachment + workload), Data Catalog (1 — entry group), Data Fusion (1 — instance), Dataflow (1 — job), Dataproc (10 — cluster + cluster IAM member + autoscaling policy + Metastore service + service IAM member + federation + federation IAM member + GDC service instance + session template + GDC application environment), Dataplex (27 — Data Lineage project config +  data product + data-product data asset + IAM member + Universal Catalog entry group / entry type / entry / entry link / aspect type + metadata feed + business glossary / category / term + lake + zone + asset + asset IAM member + datascan + lake task + their IAM members), Dialogflow (2 — SIP trunk + agent), Sensitive Data Protection / DLP (4 — inspect template + de-identify template + stored info type + job trigger), Discovery Engine (6 — data store + search engine + IAM member / binding / policy + license config), Compute (88 — including sole-tenant node templates/groups, Interconnect groups, regional commitments, Cloud Router, Cloud NAT, VPC peering, classic + HA VPN gateways/tunnels, Shared VPC host/service attach, and the full L7 Application LB stack: forwarding rules + target proxies + URL maps + backend services + managed/self-managed SSL + MIG + Autoscaler + NEG + Cloud Armor + SSL Policy + health checks + SSL/TCP proxies + PSC + regional Armor/SSL + network endpoints + security policy rules + zonal disk / region disk / regional instant snapshot / snapshot IAM member / binding / policy + Cloud Armor tier + default network tier + snapshot settings + static route + project metadata item + network firewall policy + regional network firewall policy / rule / IAM member + resource policy + disk resource policy attachment + zonal instance settings + preview feature), DNS (6 — managed zone + zone IAM + policy + record set + response policy + rule), Edge Container (3 — cluster + node pool + VPN connection), Edge Network (3 — network + subnet + interconnect attachment), Eventarc (7 — trigger + channel + channel config + enrollment + API source + message bus + pipeline), Firebase App Check (8), Firebase App Hosting (5), Firebase Data Connect (1), Firebase Remote Config (1), Cloud Filestore (3 — instance + backup + snapshot), Firestore (5), IAM (8 — service accounts + project IAM + project audit config + workload identity pool + provider), Identity Platform (2 — project config + tenant), Application Integration (1 — client), Integration Connectors (3 — connection + endpoint attachment + managed zone; node hours are never_apply), IAP (9 — settings + tunnel destination group + web backend service IAM binding + member + App Engine service / version / app IAM members + agent registry / location web IAM members), KMS (6), License Manager (1 — configuration), Looker (1 — instance), Managed Lustre (1 — instance), Parallelstore (1 — instance), Migration Center (10 — settings + source + discovery client + import job + import data file + group + preference set + report config + report + assets export job), Model Armor (1 — template), Logging (11 — metric + sinks + bucket / view / IAM / exclusion / saved query / log scope / linked dataset), Network (18 — NCC hub + CCI transport + ULL mirroring engine / collector / rule + address group + URL list + client/server TLS policies + gateway security policy + backend authentication config + connectivity test + VPC Flow Logs config + Network Services Mesh + DNS threat detector + Network Services gateway / Secure Web Gateway + Cloud NGFW firewall endpoint / association), OS Config (2 — OS policy assignment + patch deployment), Oracle Database@Google Cloud (12 — Autonomous Database + DB System + ODB network + subnet + GoldenGate deployment + connection + assignment + Exadata infrastructure + VM cluster + ExaDB VM cluster + Exascale storage vault + Exascale config), Monitoring (10 — alert policy + notification channel + uptime check + dashboard + metric descriptor / service + SLO / group / custom service / monitored project), project service enablement + usage export (2), Pub/Sub (6 — topic / subscription / 2 iam_member + schema + schema_iam_member), Pub/Sub Lite (3 — reservation + topic + subscription), Security Command Center (1 — v2 project notification config), reCAPTCHA Enterprise (1 — key), Memorystore Memcached (1 — instance), Memorystore Redis (3 — instance + Cluster + user-created connections), Memorystore for Valkey (2 — instance + user-created endpoints), Managed Service for Apache Kafka (5 — cluster + topic + ACL + Connect cluster + connector), Google Cloud NetApp Volumes (11 — storage pool + volume + snapshot + backup vault + backup policy + active directory + backup + host group + kmsconfig + volume quota rule + volume replication), Cloud Workstations (5 — cluster + config + workstation + config/workstation IAM members), Cloud Composer (3 — environment + user workloads ConfigMap/Secret), Cloud Spanner (2 — instance + database), Secret Manager (6 — secret / version / secret IAM member + regional secret / regional version / regional secret IAM member), Service Networking (2 — connection + VPC access connector), Cloud Storage (7 — bucket / bucket_object / bucket_iam_member / bucket_iam_binding + notification + hmac_key + managed folder), Storage Control (1 — project Intelligence config), Resource Manager Tags (5 — tag key / tag value / tag binding + tag key/value IAM member), Workflows (1 — workflow), Service Directory (5 — namespace / service / endpoint + namespace/service IAM member), Essential Contacts (1 — contact), Document AI (2 — processor + schema), Cloud Healthcare (11 — dataset / DICOM store / consent store / HL7v2 store / FHIR store / Data Mapper workspace + dataset / DICOM / HL7v2 / consent / FHIR IAM members), Cloud Observability (1 — trace scope), Parameter Manager (4 — parameter / version / regional parameter / regional version), BigLake Metastore (6 — catalog / database / table + iceberg catalog / namespace / table), Colab Enterprise (4 — runtime + runtime template + template IAM + schedule), Cloud Source Repositories (2 — repository + repository IAM member), Vertex AI Workbench / Notebooks (5 — environment + instance + runtime + instance/runtime IAM members), Vertex AI Workbench instances (2 — instance + IAM member), Vertex AI (21 — feature group + feature + Feature Online Store + FeatureView + RAG Engine config + Reasoning Engine + Model Garden endpoint deploy + managed dataset + Tensorboard + experiment + run + schedule + cache config + endpoint + deployment resource pool + Featurestore + entity type + feature + index + index endpoint + deployed index), Vector Search (2 — collection + index), Cluster Director / Hypercompute Cluster (1 — cluster), VMware Engine (9 — private cloud + network + network policy + network peering + cluster + subnet + datastore + external access rule + external address), Secure Source Manager (6 — instance + repository + instance/repository IAM members + hook + branch rule), Gemini for Google Cloud (8 — enablement / logging / release channel / data-sharing-with-Google settings + their project bindings), and the `google_project` data source. CI verifies regeneration is byte-deterministic via `terradart wrap --check`.
...
- [IAP settings](examples/iap_settings_quickstart/)
