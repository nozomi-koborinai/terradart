...
[`terradart_google`](packages/terradart_google/README.md) ships **897 curated resource factories + 1 data source** (898 catalog entries): Managed Microsoft Active Directory (1 — domain; never_apply hourly), Agent Identity / Agent Registry (3 — auth provider + registry binding + registry service; debt-only), AlloyDB (4 — cluster + instance + user + backup), API Keys (1 — key), App Engine (8 — application + URL dispatch + domain mapping + firewall rule + standard/flex versions + service network/traffic settings), Artifact Registry (6 — project config + repository + repository IAM member / binding / policy + download rule), Bigtable (14 — instance + table + app profile + GC policy + authorized/logical/materialized views + schema bundle + instance/table IAM member / binding / policy), Binary Authorization (5 — policy + attestor + attestor IAM member / binding / policy), Blockchain Node Engine (1 — node), Container Analysis (5 — note + note IAM member / binding / policy + occurrence), BigQuery (39 — standalone dataset access + dataset / table / dataset_iam_member / binding / policy / table_iam_member / binding / policy + job / routine / routine_iam_member / binding / policy / data_transfer_config / reservation / reservation group / capacity_commitment / connection + connection IAM member / binding / policy + assignment / row access / data policy / datapolicy IAM member / binding / policy + Analytics Hub exchange/listing IAM member / binding / policy / subscription / query template + BI reservation), Chronicle (21 — custom list + native dashboard + dashboard chart + environment + rule + rule deployment + retrohunt + reference list + watchlist + data export + parser + findings refinement + deployment + data access label/scope + data table/row + parser extension + feed + environment group + BigQuery export), Certificate Manager (6 — DNS authorization / certificate / certificate map / map entry / trust config / issuance config), Private CA (7 — CA pool + pool IAM member / binding / policy + certificate authority + certificate + certificate template), Public CA (1 — ACME external account key), Cloud Build (4 — v2 SCM connection / v2 repository / trigger + worker pool), Config (1 — deployment), Cloud Deploy (12 — delivery pipeline / target / custom target type + their IAM member / binding / policy), Cloud Functions (4 — function + function IAM member / binding / policy), Cloud IDS (1 — endpoint), Cloud Quotas (2 — quota adjuster settings + quota preference; never_apply exclude_delete), Cloud Run v2 (12 — service / job / worker pool + their IAM member / binding / policy), Cloud Scheduler (1), Cloud SQL (5 — instance / database / user + SSL cert + source representation), Cloud Tasks (4 — queue + queue IAM member / binding / policy), Contact Center Insights (8 — encryption spec + analysis rule + view + QA scorecard / revision / question + assessment rule + auto-labeling rule), Container / GKE (7 — cluster + node pool + fleet + membership + fleet scope + fleet namespace + hub feature), GKE on AWS (2 — cluster + node pool), GKE on Azure (3 — client + cluster + node pool), GKE attached clusters (1 — cluster), GKE on-prem / GDC (6 — bare metal + VMware admin/user clusters + node pools; physical/vSphere + GDC fees are never_apply), GKE Backup (10 — plans + channels + plan/restore-plan IAM member / binding / policy), API Hub (6 — host registration + instance + runtime project attachment + curation + plugin + plugin instance; debt/never_apply), Apigee (37 — organization + instance + instance attachment + environment + envgroup + envgroup attachment + environment addons + org addons + data collector + datastore + NAT address + endpoint attachment + DNS zone + space + sync authorization + target server + developer + developer app + API product + app group + env keystore + env references + environment KVM + KVM entries + keystore self-signed / key+cert / PKCS12 aliases + API proxy + shared flow + shared flow deployment + flow hook + environment debug mask + API deployment + control plane access + environment API revision deployment + security feedback + security profile v2), Backup and DR (6 — vault + plan + association + management server + service config + restore), Access Context Manager (6 — access policy + access level + service perimeter + policy IAM member / binding / policy), BeyondCorp (5 — app gateway + connector + connection + security gateway + security gateway application), App Hub (5 — application + boundary + service + service-project attachment + workload), Data Catalog (1 — entry group), Data Fusion (1 — instance), Dataflow (1 — job), Dataproc (18 — cluster + cluster IAM member / binding / policy + classic job + autoscaling policy + Metastore service / federation + their IAM member / binding / policy + GDC service instance + session template + GDC application environment + Serverless batch), Dataplex (47 — Data Lineage project config +  data product + data-product data asset + data product IAM member / binding / policy + Universal Catalog entry group / entry type / entry / entry link / aspect type + entry group/type/aspect IAM member / binding / policy + metadata feed + business glossary / category / term + glossary IAM member / binding / policy + lake + zone + asset + lake/zone/asset IAM member / binding / policy + datascan + lake task + datascan/task IAM member / binding / policy), Dialogflow (17 — SIP trunk + ES agent + full CX surface: agent / flow / intent / page / playbook / webhook / generative settings / security settings / generator / tool / entity type / environment / version / tool version / test case), Sensitive Data Protection / DLP (4 — inspect template + de-identify template + stored info type + job trigger), Discovery Engine (11 — data store + search engine + IAM member / binding / policy + license config + user store + widget config + assistant + chat / recommendation engines), Compute (112 — including sole-tenant node templates/groups, Interconnect groups, Cross-Site / wire groups, packet mirroring, regional commitments, reservations, Cloud Router, Cloud NAT, VPC peering, classic + HA VPN gateways/tunnels, Shared VPC host/service attach, and the full L7 Application LB stack: forwarding rules + target proxies + URL maps + backend services + managed/self-managed SSL + MIG + Autoscaler + NEG + Cloud Armor + SSL Policy + health checks + SSL/TCP proxies + PSC + regional Armor/SSL + network endpoints + security policy rules + zonal disk / disk IAM member / binding / policy / region disk / regional instant snapshot / snapshot IAM member / binding / policy / instance IAM member / binding / policy / subnetwork IAM member / binding / policy + Cloud Armor tier + default network tier + snapshot settings + static route + project metadata item + hierarchical firewall policy / association / rule / IAM member / binding / policy + network firewall policy + regional network firewall policy / rule / IAM member + resource policy + disk resource policy attachment + zonal instance settings + preview feature + rollout plan + global VM extension policy + regional MIG resize request), DNS (8 — managed zone + zone IAM member / binding / policy + DNS policy + record set + response policy + rule), Edge Container (3 — cluster + node pool + VPN connection), Edge Network (3 — network + subnet + interconnect attachment), Eventarc (7 — trigger + channel + channel config + enrollment + API source + message bus + pipeline), Firebase App Check (8), Firebase App Hosting (5), Firebase Data Connect (1), Firebase Remote Config (1), Cloud Filestore (3 — instance + backup + snapshot), Firestore (5), IAM (12 — service accounts + service account IAM member / binding / policy + project IAM member / binding / policy + project audit config + workload identity pool + provider), Identity Platform (2 — project config + tenant), Application Integration (1 — client), Integration Connectors (3 — connection + endpoint attachment + managed zone; node hours are never_apply), IAP (29 — settings + tunnel destination group + web backend service IAM member / binding / policy + App Engine service / version / app IAM member / binding / policy + agent registry IAM member / binding / policy + agent / endpoint / mcp_server IAM member / binding / policy + location web IAM members + location web IAM binding / policy), KMS (11 — including EKM connection + key ring / crypto key IAM member / binding / policy; EKM is never_apply / exclude_delete), License Manager (1 — configuration), Looker (1 — instance), Managed Lustre (1 — instance), Parallelstore (1 — instance), Migration Center (10 — settings + source + discovery client + import job + import data file + group + preference set + report config + report + assets export job), Model Armor (1 — template), Logging (13 — metric + sinks + bucket / view / view IAM member / binding / policy / exclusion / saved query / log scope / linked dataset), Network (45 — NCC hub + CCI transport + multicloud data-transfer config / destination + Cloud Multicast domain / group / activation / group range / activations / consumer & producer associations / group activations + ULL mirroring engine / collector / rule + OOB intercept deployment / group / endpoint group / association + OOB mirroring deployment / group / endpoint group / endpoint / association + address group + URL list + client/server TLS policies + gateway security policy + backend authentication config + connectivity test + VPC Flow Logs config + Network Services Mesh + DNS threat detector + Network Services Agent Gateway + gateway / Secure Web Gateway + Cloud NGFW firewall endpoint / association + TLS inspection policy + security profile / profile group + Media CDN Edge Cache service / origin / keyset), OS Config (2 — OS policy assignment + patch deployment), Oracle Database@Google Cloud (12 — Autonomous Database + DB System + ODB network + subnet + GoldenGate deployment + connection + assignment + Exadata infrastructure + VM cluster + ExaDB VM cluster + Exascale storage vault + Exascale config), Monitoring (10 — alert policy + notification channel + uptime check + dashboard + metric descriptor / service + SLO / group / custom service / monitored project), project service enablement + usage export (2), Pub/Sub (12 — topic / subscription / topic IAM member / binding / policy + schema / schema IAM member / binding / policy + subscription IAM member / binding / policy), Pub/Sub Lite (3 — reservation + topic + subscription), Security Command Center (1 — v2 project notification config), reCAPTCHA Enterprise (1 — key), Memorystore Memcached (1 — instance), Memorystore Redis (3 — instance + Cluster + user-created connections), Memorystore for Valkey (2 — instance + user-created endpoints), Managed Service for Apache Kafka (5 — cluster + topic + ACL + Connect cluster + connector), Google Cloud NetApp Volumes (11 — storage pool + volume + snapshot + backup vault + backup policy + active directory + backup + host group + kmsconfig + volume quota rule + volume replication), Cloud Workstations (9 — cluster + config + workstation + config/workstation IAM member / binding / policy), Cloud Composer (3 — environment + user workloads ConfigMap/Secret), Cloud Spanner (4 — instance + database + instance partition + backup schedule; partition compute and backup storage are never_apply), Secret Manager (10 — secret / version / secret IAM member / binding / policy + regional secret / regional version / regional secret IAM member / binding / policy), Service Networking (2 — connection + VPC access connector), Cloud Storage (9 — bucket / bucket_object / bucket_iam_member / bucket_iam_binding / bucket_iam_policy + notification + hmac_key + managed folder + Anywhere Cache; cache capacity is never_apply), Storage Control (1 — project Intelligence config), Resource Manager Tags (9 — tag key / tag value / tag binding + tag key/value IAM member / binding / policy), Workflows (1 — workflow), Service Directory (9 — namespace / service / endpoint + namespace/service IAM member / binding / policy), Essential Contacts (1 — contact), Document AI (2 — processor + schema), Cloud Healthcare (21 — dataset / DICOM store / consent store / HL7v2 store / FHIR store / Data Mapper workspace + dataset / DICOM / FHIR / HL7v2 / consent IAM member / binding / policy), Cloud Observability (1 — trace scope), Parameter Manager (4 — parameter / version / regional parameter / regional version), BigLake Metastore (6 — catalog / database / table + iceberg catalog / namespace / table), Colab Enterprise (7 — runtime + runtime template + template IAM member / binding / policy + schedule + notebook execution; runtime/execution compute is never_apply), Cloud Source Repositories (4 — repository + repository IAM member / binding / policy), Vertex AI Workbench / Notebooks (9 — environment + instance + runtime + instance/runtime IAM member / binding / policy), Vertex AI Workbench instances (4 — instance + IAM member / binding / policy), Vertex AI (23 — feature group + feature + Feature Online Store + FeatureView + RAG Engine config + Reasoning Engine + Model Garden endpoint deploy + managed dataset + Tensorboard + experiment + run + schedule + cache config + endpoint + deployment resource pool + persistent resource + semantic governance policy engine + Featurestore + entity type + feature + index + index endpoint + deployed index), Vector Search (3 — collection + data object + index), Cluster Director / Hypercompute Cluster (1 — cluster), VMware Engine (9 — private cloud + network + network policy + network peering + cluster + subnet + datastore + external access rule + external address), Secure Source Manager (10 — instance + repository + instance/repository IAM member / binding / policy + hook + branch rule), Gemini for Google Cloud (11 — enablement / logging / release channel / data-sharing-with-Google settings + their project bindings + Code Assist repository index / code tools setting + binding), and the `google_project` data source. CI verifies regeneration is byte-deterministic via `terradart wrap --check`.
