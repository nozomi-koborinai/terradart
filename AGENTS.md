...
- **Cost-classify the new resource type.** Add `<type>: <tier>` to `tool/apply_cost_denylist.yaml` — `safe` (no existence billing, cheap), `sweep_only` (hourly/expensive; also add the example to `apply_smoke_pr_skip.yaml`), or `never_apply` (existence-billed like software licenses/reservations; also add the example to `apply_smoke_skip.yaml`). Confirm billing via terraform MCP + the provider docs. **When unsure, leave it unclassified** — the example stays skipped (safe) until classified. Never mark a billable type `safe`.
