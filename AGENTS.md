...
The cost gate is **default-deny**: a type absent from all three tiers in `tool/apply_cost_denylist.yaml` (`safe`/`sweep_only`/`never_apply`) is treated as unclassified and is NOT applied — `apply_smoke.sh` skips the example at runtime and `apply_smoke_test.sh` (test9) fails the build. A second sanity check (test12) fails any `safe` type whose name matches a high-cost pattern (`license`/`reservation`/…). So a newly curated resource type must be cost-classified before its example can apply.
