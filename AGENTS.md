...
- **Cost-classify the new resource type.** Add `<type>: <tier>` to `tool/apply_cost_denylist.yaml` — `safe` (no existence billing, cheap), `sweep_only` (hourly/expensive; also add the example to `apply_smoke_pr_skip.yaml`), or `never_apply` (existence-billed like software licenses/reservations; also add the example to `apply_smoke_skip.yaml`). Confirm cost on two axes — (1) price via the gcp-cost MCP (list_skus / get_sku_price), and (2) billing behavior via terraform MCP + docs (existence-billed vs hourly, and whether destroy stops the charge); record the basis in the ledger comment. The gcp-cost price is list-price only, and special services like License Manager / Office SPLA are absent from the Cloud Billing Catalog — fall back to docs/pattern for those. **When unsure, leave it unclassified** — the example stays skipped (safe) until classified. Never mark a billable type `safe`.
...
| Cost-classify a type (gcp-cost MCP) | `tool/agent_cost_classify.sh <google_type> [BillingCatalogServiceName]` |
