...
- **`ModuleCall` / `Stack.addModule`** — a `module "<name>" { ... }` call as a Dart value: `source`, `version`, `inputs`, `providers`, `count` / `for_each` and `depends_on`, emitted under the top-level `module` key in registration order. A module's outputs are read back as `TfRef`s (`call.output<String>('member')` → `${module.<name>.member}`), so they flow into any `TfArg` slot, `depends_on`, `lifecycle` and exports. Synth validates what Terraform would: an input named like a meta-argument is rejected at construction, a `providers` value must name a registered provider configuration, and `var.<name>` references inside the inputs are checked like a resource's. A repeated local name throws `DuplicateModuleError` (#665).
