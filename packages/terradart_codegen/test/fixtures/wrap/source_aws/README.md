...
Full catalog at the current pin (`6.66.0`): every resource and data source.
...
dart tool/extract_schema_subset.dart \
  ...
  --resources-from=packages/terradart_codegen/test/fixtures/wrap/source_aws/schema.json \
  --out=packages/terradart_codegen/test/fixtures/wrap/source_aws
```

To ADD a resource, append it via union:
`--resources-from=packages/terradart_codegen/test/fixtures/wrap/source_aws/schema.json --resources=<new_type>`.
