...
''');
  ...
  group(
      'production round-trip (20 entries — Phase 4.1 13 + Phase 4.5 Wave 0+1 7)',
      () {
    test(
      'lib/src/codegen/wrapper_overrides/yaml/ loads 19 resources + 1 data source',
      () {
        final loaded = loadWrapperOverrides(
          rootDir:
              p.absolute('lib', 'src', 'codegen', 'wrapper_overrides', 'yaml'),
        );
        expect(loaded.resources.length, 19);
