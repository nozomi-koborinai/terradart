...
- **`terradart wrap`** — used by `terradart_google` maintainers to regenerate the 28 Tier 1 factory wrappers + their schema carriers from curated YAML overrides under `lib/src/codegen/wrapper_overrides/yaml/`. Run with `--check` in CI to verify the wrap output stays byte-identical (deterministic codegen guarantee).
...
## Quickstart: emit bindings for a Tier 2 resource
