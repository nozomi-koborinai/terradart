...
It matches Terraform type strings against the static catalogs compiled into the five provider packages (`package:terradart_<provider>/catalog.dart`). Anything it can't analyze (a registry/git module, a file it can't parse) is reported explicitly rather than dropped silently.
