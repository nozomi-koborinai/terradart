...
///
/// Multiple [InstanceGroupManagerVersion] entries enable canary
...
class InstanceGroupManagerVersionTargetSize {
  const InstanceGroupManagerVersionTargetSize({this.fixed, this.percent});
...
/// [InstanceGroupManagerVersion] across its members.
@immutable
...
class InstanceGroupManagerNamedPort {
  const InstanceGroupManagerNamedPort({required this.name, required this.port});
...
@immutable
class InstanceGroupManagerStatefulIp {
  const InstanceGroupManagerStatefulIp({this.interfaceName, this.deleteRule});
...
class InstanceGroupManagerAllInstancesConfig {
  const InstanceGroupManagerAllInstancesConfig({this.labels, this.metadata});
...
class InstanceGroupManagerStandbyPolicy {
  const InstanceGroupManagerStandbyPolicy({this.initialDelaySec, this.mode});
...
class InstanceGroupManagerTargetSizePolicy {
  const InstanceGroupManagerTargetSizePolicy({required this.mode});
...
class InstanceGroupManagerResourcePolicies {
  const InstanceGroupManagerResourcePolicies({this.workloadPolicy});
...
/// - At least one [InstanceGroupManagerVersion] in [versions]; each
///   version requires an `instance_template` self-link.
///
/// Cross-resource references (typical wiring):
/// - [InstanceGroupManagerVersion.instanceTemplate]: self-link of a
...
/// - [InstanceGroupManagerAutoHealingPolicy.healthCheck]: self-link of a
...
///   longer than [InstanceGroupManagerAutoHealingPolicy.initialDelaySec],
...
///   ],
///   namedPorts: const [
///     InstanceGroupManagerNamedPort(name: 'http', port: 80),
///   ],
...
///     initialDelaySec: 300,
///   ),
///   updatePolicy: const InstanceGroupManagerUpdatePolicy(
...
final class GoogleComputeInstanceGroupManager extends Resource {
  ...
  GoogleComputeInstanceGroupManager({
    ...
    List<InstanceGroupManagerTargetSizePolicy>? targetSizePolicies,
    ...
    List<InstanceGroupManagerStatefulIp>? statefulInternalIps,
    List<InstanceGroupManagerStatefulIp>? statefulExternalIps,
