...
- [ ] 5b. **New barrel?** Add its entry to `packages/terradart_codegen/lib/src/codegen/barrels/barrels.yaml` (`doc:` required; `file:` / `extraExports:` only when needed). Barrel files under `terradart_google/lib/` are generated by `wrap` from the catalog — never hand-edit them; `wrap` fails closed on a catalog barrel with no manifest entry.
- [ ] 6. **Lint:** `cd packages/terradart_codegen && dart run bin/terradart.dart lint-override` (must exit 0). A `migrate-shape-underivable` finding means the migration manifest cannot derive a `prelude` helper or custom slot (#659): reshape the helper to a field-per-key `encode()` map literal, declare `customSlots.<slot>.migrate: {kind: manual, reason: ...}` when the slot is manual by design, or add a reasoned `tool/migrate_manifest_debt.yaml` entry — the last two are maintainer decisions, outside the Wave scope ledger.
- [ ] 7. **Regenerate:** `dart run bin/terradart.dart wrap --only <terraform_type> ...` for the wrapper, then a full `wrap` **with `--migrate-manifest`** — the migration manifest (`packages/terradart_migrate/lib/src/manifest/google.g.dart`) is a whole-registry artifact that `--only` skips, every curated factory adds a `MigrateEntry` to it, and the `wrap --check` lane fails while it is stale — then `wrap --check` against the fixture paths in `AGENTS.md` **Useful Commands**.
...
- [ ] 10. **Wave / example:** if this resource is part of a Wave release, follow [`terradart-ship-wave`](../terradart-ship-wave/SKILL.md). For new `*IamBinding` / `*IamPolicy` when the sibling `*IamMember` is already curated and in quickstart synth, prefer `tool/example_debt.yaml` with `iam-adjunct-debt:` over extending examples; otherwise add or extend a quickstart. `curatedDoc` alone is not enough.
- [ ] 10b. **Gated example check:** if applying the new type needs an organization, an entitlement or real external inputs, bills while it exists, or leaves behind something that cannot be deleted (or a name that stays reserved), its example must be a **gated example** (README `## Before you apply` section — see `AGENTS.md` **Example verification**). Extend the product's gated example or add one; never put such a factory into an example without that section. Cost or apply-ability is never a reason for `tool/example_debt.yaml`.
