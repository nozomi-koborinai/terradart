...
- [ ] 10b. **Gated example check:** if applying the new type needs an organization, an entitlement, real external inputs, or bills while it exists, its example must be a **gated example** (README `## Before you apply` section — see `AGENTS.md` **Example verification**). Extend the product's gated example or add one; never put such a factory into an example without that section. Cost or apply-ability is never a reason for `tool/example_debt.yaml`.
