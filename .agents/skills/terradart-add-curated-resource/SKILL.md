...
- [ ] 10b. **Cost-classify** every Terraform type that appears in the example synth output (including dependency types like `google_storage_bucket`). In Cursor, call the **gcp-cost MCP** tools (`list_services`, `list_skus`, `get_sku_price`) and record `<type>: <tier>  # gcp-cost: …; billing-behavior: …` in [`tool/apply_cost_denylist.yaml`](../../../tool/apply_cost_denylist.yaml). IAM adjuncts with no catalog SKU may use `billing-behavior:` instead of `gcp-cost:`. Do not hand-classify from memory; do not wrap MCP in repo shell scripts. `apply_smoke_test.sh` test 13 fails hand-wavy `safe` comments.
